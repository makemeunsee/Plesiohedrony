...
class BitruncatedCubicHoneycomb extends Honeycomb {
  ...
  def polyhedron(i: Int, j: Int, k: Int, polyId: Int): Polyhedron =
    TruncatedOctahedron(i, j, k)
  def polyhedrons(i: Int, j: Int, k: Int): Iterable[Polyhedron] = Iterable(
    TruncatedOctahedron(i, j, k)
  )
}
...
object BitruncatedCubicHoneycomb {
  ...
  private[models] object Faces {
    ...
    trait BTHFace {
      ...
      def opposite(i: Int, j: Int, k: Int) = {
        val s = %+(
          k,
          2
        ) // on odd k levels, neighbors are shifted by one on the xy plan
        id match {
          ...
          case TopNorthEastHexa.id =>
            FaceId(i + s, j + s, k + 1, BottomSouthWestHexa)
          case TopNorthWestHexa.id =>
            FaceId(i - 1 + s, j + s, k + 1, BottomSouthEastHexa)
          case TopSouthWestHexa.id =>
            FaceId(i - 1 + s, j - 1 + s, k + 1, BottomNorthEastHexa)
          case TopSouthEastHexa.id =>
            FaceId(i + s, j - 1 + s, k + 1, BottomNorthWestHexa)
          case BottomNorthEastHexa.id =>
            FaceId(i + s, j + s, k - 1, TopSouthWestHexa)
          case BottomNorthWestHexa.id =>
            FaceId(i - 1 + s, j + s, k - 1, TopSouthEastHexa)
          case BottomSouthWestHexa.id =>
            FaceId(i - 1 + s, j - 1 + s, k - 1, TopNorthEastHexa)
          case BottomSouthEastHexa.id =>
            FaceId(i + s, j - 1 + s, k - 1, TopNorthWestHexa)
        }
      }
    }
    ...
    object TopSquare
        extends Face(
          0,
          new Quad(List(TopSquareN, TopSquareW, TopSquareS, TopSquareE))
        )
        with BTHFace
    object BottomSquare
        extends Face(
          0,
          new Quad(
            List(BottomSquareE, BottomSquareS, BottomSquareW, BottomSquareN)
          )
        )
        with BTHFace
    object NorthSquare
        extends Face(
          0,
          new Quad(List(NorthSquareU, NorthSquareE, NorthSquareD, NorthSquareW))
        )
        with BTHFace
    object SouthSquare
        extends Face(
          0,
          new Quad(List(SouthSquareU, SouthSquareW, SouthSquareD, SouthSquareE))
        )
        with BTHFace
    object EastSquare
        extends Face(
          0,
          new Quad(List(EastSquareD, EastSquareN, EastSquareU, EastSquareS))
        )
        with BTHFace
    object WestSquare
        extends Face(
          0,
          new Quad(List(WestSquareU, WestSquareN, WestSquareD, WestSquareS))
        )
        with BTHFace
    object TopNorthEastHexa
        extends Face(
          0,
          new Hexad(
            List(
  ...
  case class TruncatedOctahedron(val i: Int, val j: Int, val k: Int)
      extends Polyhedron {
