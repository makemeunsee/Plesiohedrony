...
class TetraOctaHoneycomb extends Honeycomb {
  ...
  def polyhedrons(i: Int, j: Int, k: Int) =
    polyhedronIds.map(id => polyhedron(i, j, k, id))
}
...
object TetraOctaHoneycomb {
  ...
  private[models] object Points {
    ...
    object LowSouth
        extends TOHPoint(0.5f, -SQRT_3_BY_3 - SQRT_3_BY_2, -SQRT_6_BY_3)
  }
  import Points._
  ...
  private[models] object Faces {
    ...
    // octahedron faces
    object O_Top
        extends Face(octaId, new Triangle(South, East, West))
        with TetraOctaFace
    object O_Bottom
        extends Face(octaId, new Triangle(LowEast, LowWest, LowNorth))
        with TetraOctaFace
    object O_NorthEast
        extends Face(octaId, new Triangle(LowNorth, East, LowEast))
        with TetraOctaFace
    object O_North
        extends Face(octaId, new Triangle(West, East, LowNorth))
        with TetraOctaFace
    object O_NorthWest
        extends Face(octaId, new Triangle(West, LowNorth, LowWest))
        with TetraOctaFace
    object O_SouthWest
        extends Face(octaId, new Triangle(West, LowWest, South))
        with TetraOctaFace
    object O_South
        extends Face(octaId, new Triangle(South, LowWest, LowEast))
        with TetraOctaFace
    object O_SouthEast
        extends Face(octaId, new Triangle(East, South, LowEast))
        with TetraOctaFace

    // downward tetrahedron faces
    object DT_Top
        extends Face(dTetraId, new Triangle(East, North, West))
        with TetraOctaFace
    object DT_South
        extends Face(dTetraId, new Triangle(East, West, LowNorth))
        with TetraOctaFace
    object DT_NorthWest
        extends Face(dTetraId, new Triangle(North, LowNorth, West))
        with TetraOctaFace
    object DT_NorthEast
        extends Face(dTetraId, new Triangle(North, East, LowNorth))
        with TetraOctaFace

    // upward tetrahedron faces
    object UT_Bottom
        extends Face(uTetraId, new Triangle(LowWest, LowEast, LowSouth))
        with TetraOctaFace
    object UT_North
        extends Face(uTetraId, new Triangle(South, LowEast, LowWest))
        with TetraOctaFace
    object UT_SouthWest
        extends Face(uTetraId, new Triangle(LowSouth, South, LowWest))
        with TetraOctaFace
    object UT_SouthEast
        extends Face(uTetraId, new Triangle(LowEast, South, LowSouth))
        with TetraOctaFace
  }
  import Faces._

  case class Octahedron(i: Int, j: Int, k: Int)
      extends Polyhedron
      with TOHCoordinates {
    override val sourcePolyhedron = Octahedron0
  }

  case class UpwardTetrahedron(i: Int, j: Int, k: Int)
      extends Polyhedron
      with TOHCoordinates {
    override val sourcePolyhedron = UpwardTetrahedron0
  }

  case class DownwardTetrahedron(i: Int, j: Int, k: Int)
      extends Polyhedron
      with TOHCoordinates {
  ...
  object Octahedron0 extends Octahedron(0, 0, 0) {
    ...
    override lazy val center =
      faces.foldLeft(new Point3f(0, 0, 0))((z, e) => z + e.polygon.center / 8)
  }

  object UpwardTetrahedron0 extends UpwardTetrahedron(0, 0, 0) {
    override val id = uTetraId
    override val name = "UpwardTetrahedron0"
    override lazy val faces =
      List(UT_Bottom, UT_North, UT_SouthEast, UT_SouthWest)
    override lazy val center =
      faces.foldLeft(new Point3f(0, 0, 0))((z, e) => z + e.polygon.center / 4)
  }
  ...
  object Grid {
    ...
    sealed abstract class Tile(i: Int, j: Int, src: Polygon)
        extends TOHCoordinates {
      val k = 0
