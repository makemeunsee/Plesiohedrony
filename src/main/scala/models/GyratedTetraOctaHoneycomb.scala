...
class GyratedTetraOctaHoneycomb extends Honeycomb {
  ...
  import GyratedTetraOctaHoneycomb.{
    GyratedOctahedron,
    GyratedDownwardTetrahedron,
    GyratedUpwardTetrahedron,
    Octahedron,
    DownwardTetrahedron,
    UpwardTetrahedron
  }
  ...
  def polyhedron(i: Int, j: Int, k: Int, polyId: Int) =
    (polyId, %+(k, 2)) match {
      ...
      case (TetraOctaHoneycomb.dTetraId, 1) =>
        GyratedDownwardTetrahedron(i, j, k)
...
object GyratedTetraOctaHoneycomb {
  ...
  trait GTOHPolyhedron extends Polyhedron with TOHCoordinates {
    override def origin =
      new Point3f(i + j * 0.5f, j * SQRTS.SQRT_3_BY_2, k * SQRTS.SQRT_6_BY_3)
  }
  ...
  private def ySymmetry(t: Triangle, yPlane: Float): Triangle =
    new Triangle(
      ySymmetry(t.p1, yPlane),
      ySymmetry(t.p2, yPlane),
      ySymmetry(t.p3, yPlane)
    )
  ...
  private[models] object GyratedFaces {
    ...
    private def toGyratedFace(f: Face) = {
      ...
      new Face(
        f.polyId,
        ySymmetry(new Triangle(t._1, t._3, t._2), Octahedron0.center.y)
      ) with GyratedOctaTetraFace
    }

    def toRegularFace(f: Face) = new Face(f.polyId, f.polygon)
      with GyratedOctaTetraFace
  ...
  private case class UpwardTetrahedron(i: Int, j: Int, k: Int)
      extends GTOHPolyhedron {
    override val sourcePolyhedron = UpwardTetrahedron0
    override val faces =
      List(RUT_Bottom, RUT_North, RUT_SouthWest, RUT_SouthEast)
  }

  private case class DownwardTetrahedron(i: Int, j: Int, k: Int)
      extends GTOHPolyhedron {
    override val sourcePolyhedron = DownwardTetrahedron0
    override val faces = List(RDT_Top, RDT_South, RDT_NorthWest, RDT_NorthEast)
  }

  private case class GyratedOctahedron(i: Int, j: Int, k: Int)
      extends GTOHPolyhedron {
    override val sourcePolyhedron = GyratedOctahedron0
  }

  private case class GyratedUpwardTetrahedron(i: Int, j: Int, k: Int)
      extends GTOHPolyhedron {
    override val sourcePolyhedron = GyratedUpwardTetrahedron0
  }

  private case class GyratedDownwardTetrahedron(i: Int, j: Int, k: Int)
      extends GTOHPolyhedron {
  ...
  private object GyratedUpwardTetrahedron0
      extends GyratedUpwardTetrahedron(0, 0, 0) {
    override val id = UpwardTetrahedron0.id
    override val name = "GyratedUpwardTetrahedron0"
    override val faces =
      List(GUT_Bottom, GUT_South, GUT_NorthWest, GUT_NorthEast)
    override lazy val center =
      ySymmetry(UpwardTetrahedron0.center, Octahedron0.center.y)
  }

  private object GyratedDownwardTetrahedron0
      extends GyratedDownwardTetrahedron(0, 0, 0) {
    ...
    override lazy val center =
      ySymmetry(DownwardTetrahedron0.center, Octahedron0.center.y)
  }

  private lazy val gyratedFaces =
    ((GyratedOctahedron0.faces) ++ (GyratedUpwardTetrahedron0.faces) ++ (GyratedDownwardTetrahedron0.faces))
      .map(e => (e.id, e))
      .toMap
