...
class TriakisTruncatedTetraHoneycomb extends Honeycomb {
  ...
  import util.Math.%+
  def polyhedron(i: Int, j: Int, k: Int, polyId: Int) = if (%+(polyId, 2) == 0)
    DownwardTriakisTruncatedTetra(i, j, k)
  else UpwardTriakisTruncatedTetra(i, j, k)
  def polyhedrons(i: Int, j: Int, k: Int) = List(
    DownwardTriakisTruncatedTetra(i, j, k),
    UpwardTriakisTruncatedTetra(i, j, k)
  )
}
...
object TriakisTruncatedTetraHoneycomb {
  ...
  object Points {
    ...
    val NorthWest =
      barycenter(List(NorthWestB, NorthWestS, NorthWestNE, TetraNW), 4)
    val NorthEastB = third(TetraNE, TetraBottom)
    val NorthEastS = third(TetraNE, TetraSouth)
    val NorthEastNW = third(TetraNE, TetraNW)
    val NorthEast =
      barycenter(List(NorthEastB, NorthEastS, NorthEastNW, TetraNE), 4)
  }
  import Points._
  ...
  object Faces {
    ...
    object B0
        extends Face(firstId, new Triangle(Bottom, BottomNE, BottomS))
        with TriakisFace
    object B1
        extends Face(firstId, new Triangle(Bottom, BottomNW, BottomNE))
        with TriakisFace
    object B2
        extends Face(firstId, new Triangle(Bottom, BottomS, BottomNW))
        with TriakisFace
    object S0
        extends Face(firstId, new Triangle(South, SouthB, SouthNE))
        with TriakisFace
    object S1
        extends Face(firstId, new Triangle(South, SouthNE, SouthNW))
        with TriakisFace
    object S2
        extends Face(firstId, new Triangle(South, SouthNW, SouthB))
        with TriakisFace
    object NW0
        extends Face(firstId, new Triangle(NorthWest, NorthWestB, NorthWestS))
        with TriakisFace
    object NW1
        extends Face(firstId, new Triangle(NorthWest, NorthWestS, NorthWestNE))
        with TriakisFace
    object NW2
        extends Face(firstId, new Triangle(NorthWest, NorthWestNE, NorthWestB))
        with TriakisFace
    object NE0
        extends Face(firstId, new Triangle(NorthEast, NorthEastS, NorthEastB))
        with TriakisFace
    object NE1
        extends Face(firstId, new Triangle(NorthEast, NorthEastNW, NorthEastS))
        with TriakisFace
    object NE2
        extends Face(firstId, new Triangle(NorthEast, NorthEastB, NorthEastNW))
        with TriakisFace
    ...
    object HexaSW
        extends Face(
          firstId,
          new Hexad(
            List(NorthWestS, NorthWestB, BottomNW, BottomS, SouthB, SouthNW)
          )
        )
        with TriakisFace
    object HexaN
    ...
    private val TetraFaces: Map[Face, Face] = Tetra1Faces
      .map(f =>
        (
          f,
          new Face(
            TriakisTruncatedTetra2.id,
            f.polygon
              .pointSymetry(TriakisTruncatedTetra1.center)
              .reverse
              .translate(northTranslation)
          ) with TriakisFace
        )
      )
      .toMap
  ...
  sealed trait TriakisTruncatedTetra extends Polyhedron {
    lazy val origin =
      new Point3f(
        i * 2 * southEastTranslation.x + j * southEastTranslation.x,
        j * (northTranslation.y - southEastTranslation.y) - k * northTranslation.y,
        k * upTranslation.z
      )
  }

  case class DownwardTriakisTruncatedTetra(i: Int, j: Int, k: Int)
      extends TriakisTruncatedTetra {
    val sourcePolyhedron = TriakisTruncatedTetra1
  }

  case class UpwardTriakisTruncatedTetra(i: Int, j: Int, k: Int)
      extends TriakisTruncatedTetra {
