...
class DefaultRenderer(camera: Camera, w: Int, h: Int) extends Renderer(w, h) {
  ...
  val moon = new SpecialRenderable {
    ...
    def render() {
      ...
      glTranslatef(x,0,y)
      glRotatef(angle.toDegrees,0,1,0)
  ...
  def render(picking: Boolean = false, lastHit: Option[ID] = None): Option[ID] = perfed ("render") {
    if (picking) {
      ...
      glEnable(GL_LIGHT0)
      glEnable(GL_LIGHTING)
      render(picking = false, Picking.readPicking((width / 2, height / 2), inRange, {p: Pickable => p.id}))
      //lastHit
    } else {
  ...
  protected def renderLine(r: Renderable) {
    ...
    successivePairsCycling(r.toContour.toList) foreach { case (p1, p2) =>
  ...
  private def render(r: Renderable, c: Color3B, n: Point3f) {
    ...
    r.toTriangles foreach drawTriangle
  }
  ...
  private def renderPlayers() {
    ...
    for ( player <- players.values ;
      ...
      glRotatef(player._2.toDegrees, 0, 0, -1)
      glTranslatef(-pos.x, -pos.y, -pos.z)
    
      glBegin(GL_TRIANGLES)
      playerPositionToPlayerRenderables(pos) foreach render
      glEnd()
      ...
      glBegin(GL_LINES)
      playerPositionToPlayerRenderables(pos) foreach renderLine
      glColor3ub(-1, 0, 0)
  ...
  private def renderObjects(lastHit: Option[ID]) {
    ...
    glBegin(GL_TRIANGLES)
    lastHit match {
      case None => visibles.values foreach { render(_) }
      case Some(faceId) => visibles.values.foreach { g =>
        if (g.id == faceId)
          renderPicked(g)
        else
          render(g)
    ...
    glColor3f(1,1,1)
    glBegin(GL_LINES)
    visibles.values.foreach { renderLine(_) }
    glEnd()
  }
