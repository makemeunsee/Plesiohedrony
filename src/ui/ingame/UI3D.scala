...
class UI3D(camera: Camera, name: String) extends Actor {
    ...
    new Thread {
      override def run {
        ...
        while (!finished) {
          ...
          picked map ( context.parent ! Picked(_, renderingMode) )
  ...
  def unmissable: Receive = {
    ...
    case PlayerList(l) =>
      renderer.players = renderer.players.filter { case (oId, _) => l.contains(oId) }
      l map { case (oId, name) => println(s"$name, $oId") }
  ...
  def handleInputs: Receive = {
    case NextInput => lastMouseUpdate = control(lastMouseUpdate, inputs(_))
